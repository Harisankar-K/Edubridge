...
histogram TotalCase/group=State transparency=0.5 scale=count;
density Active/type=normal group=State;
...
proc sgplot data=vaccination_data_latest;
scatter x=Active y=State;
ellipse x=Active y=State;
title 'scatter plot  of state vs active cases';
...
title "vaccination_data_latest";
proc sgplot data=vaccination_data_latest;
histogram Totalcase/filattrs= (color=red)scale=proportion;
density Totalcase;
run;    
